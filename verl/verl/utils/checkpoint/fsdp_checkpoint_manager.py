...
def _clean_none_keys_from_config(config):
    """Clean None keys from config dict to avoid JSON serialization issues.

    Some model configs may contain None as dictionary keys, which causes
    'TypeError: '<' not supported between instances of 'NoneType' and 'str''
    when json.dumps uses sort_keys=True.
    """
    ...
    def clean_dict(d):
        if not isinstance(d, dict):
            return d
        return {k: clean_dict(v) for k, v in d.items() if k is not None}

    cleaned_dict = clean_dict(config_dict)

    # Update config attributes by removing None keys if any exist
    for key in list(vars(config).keys()):
        attr = getattr(config, key, None)
        if isinstance(attr, dict) and None in attr:
            setattr(config, key, {k: v for k, v in attr.items() if k is not None})
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            # Clean None keys from config to avoid JSON serialization issues
            model_config = _clean_none_keys_from_config(model_config)
